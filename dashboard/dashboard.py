...
if page == "Tren Penyewaan Sepeda":
    ...
    year_selected = st.selectbox("Pilih Tahun", [2011, 2012], key="main_year")
    df_filtered = day_df[day_df["year"] == year_selected]

    st.subheader(f"📈 Tren Penyewaan Sepeda per Bulan ({year_selected})")
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.lineplot(x=df_filtered["mnth"],
                 y=df_filtered["cnt"], marker="o", ax=ax)
